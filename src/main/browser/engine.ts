...
export class ElectronBrowserBackend implements BrowserBackend {
  ...
  /**
   * Return the underlying WebContentsView if one has been created (i.e. if the
   * browser has been used), or null if it hasn't been lazily initialised yet.
   *
   * Used by BrowserSurface to attach the view visibly to the app window. The
   * view is NOT a new one — it's the same object navigate/screenshot operate
   * on, so attaching it does not change the agent's headless behavior.
   */
  getWebContentsView(): ElectronWebContentsView | null {
    return this._view;
  }
}
