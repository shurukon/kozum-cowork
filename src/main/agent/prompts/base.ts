...
Long-running commands: use \`shell_exec\` with \`timeoutSeconds\` or \`noTimeout\` when you expect a command to outlast the default, and \`shell_exec_bg\` when it will take minutes — that returns a job id immediately and you poll it, instead of blocking the whole turn.`);
  ...
  parts.push(`<specific_tools>
- file_read_image / file_read_pdf: read image and PDF contents directly into context (returns image blocks). Use them instead of file_raw when the asset is binary.
- file_move / file_copy / file_delete: filesystem operations without spawning shell_exec. Prefer them for moving, copying, or removing paths.
- file_search / glob_match: search and pattern-match inside the working folder; prefer these over shelling out to find/grep.
