...
(function () {
  ...
  const mockSettings = {
    general: {
      ...
      defaultWorkspace: "/tmp/preview-workspace",
      rules: "",
  ...
  window.kozum = {
    ...
    providers: {
      ...
      refreshModels: async () => ok({ models: [{ id: "claude-sonnet-4-20250514", displayName: "Claude Sonnet 4", contextWindow: 200000, vision: "yes" as const }], warning: null }),
      listModels: async () => [{ id: "claude-sonnet-4-20250514", displayName: "Claude Sonnet 4", contextWindow: 200000, vision: "yes" as const }],
      addCustom: async (input) => ok({ id: "custom_1", name: input.name, protocol: "openai-chat", baseUrl: input.baseUrl, authScheme: "bearer", modelsPath: "/models", staticModels: [input.modelId], builtIn: false }),
      addModel: async (_providerId, modelId) => ok({ id: "custom_1", name: "Custom", protocol: "openai-chat", baseUrl: "https://example.com", authScheme: "bearer", modelsPath: "/models", staticModels: [modelId], builtIn: false }),
      removeModel: async () => ok({ id: "custom_1", name: "Custom", protocol: "openai-chat", baseUrl: "https://example.com", authScheme: "bearer", modelsPath: "/models", staticModels: ["model"], builtIn: false }),
    ...
    sessions: {
      list: async () => [],
      get: async () => null,
      create: async (mode, selection) => ok({ id: "session-preview", mode, title: "Preview Session", selection, permissionMode: mode === "code" ? "ask_permission" : "accept_edits", workingFolder: null, projectId: null, createdAt: Date.now(), updatedAt: Date.now(), archived: false }),
    ...
    mcp: {
      ...
      tools: async () => [],
      setToolPolicy: async (_id, policy) => ok({ id: "mcp-1", name: "server", enabled: true, transport: "http", hasAuthToken: false, createdAt: Date.now(), installedByAgent: false, status: "disconnected", toolCount: 0, toolPolicy: policy }),
    },
    plugins: {
      list: async () => [],
      setEnabled: async () => ok(undefined),
