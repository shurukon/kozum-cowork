...
import type { AgentEvent, TokenUsage } from "../../src/shared/types.ts";
...
function withTurnStart(
  ...
  return apply(base, { type: "turn_start", sessionId, messageId, model });
}
...
describe("applyEventToMode — turn_start", () => {
  ...
  it("clears the tool card map", () => {
    let s = fresh();
    s = apply(s, { type: "turn_start", sessionId: "s0", messageId: "prev", model: "m" });
    s = apply(s, {
      type: "tool_start",
      sessionId: "s0",
      toolUseId: "old-t",
...
describe("applyEventToMode — text_delta", () => {
  it("accumulates deltas into streamingText", () => {
    let s = withTurnStart();
    s = apply(s, { type: "text_delta", sessionId: "s1", messageId: "m1", delta: "Hello" });
    s = apply(s, { type: "text_delta", sessionId: "s1", messageId: "m1", delta: " world" });
    assert.equal(s.streamingText, "Hello world");
  });

  it("updates the text block in the message's content", () => {
    let s = withTurnStart();
    s = apply(s, { type: "text_delta", sessionId: "s1", messageId: "m1", delta: "Hi" });
    s = apply(s, { type: "text_delta", sessionId: "s1", messageId: "m1", delta: "!" });
...
describe("applyEventToMode — thinking_delta", () => {
  it("accumulates deltas into streamingThinking", () => {
    let s = withTurnStart();
    s = apply(s, { type: "thinking_delta", sessionId: "s1", messageId: "m1", delta: "Think" });
    s = apply(s, { type: "thinking_delta", sessionId: "s1", messageId: "m1", delta: "ing..." });
    assert.equal(s.streamingThinking, "Thinking...");
  });

  it("creates a thinking block in the message content", () => {
    let s = withTurnStart();
    s = apply(s, { type: "thinking_delta", sessionId: "s1", messageId: "m1", delta: "deep" });
...
describe("applyEventToMode — tool_start / tool_progress / tool_end", () => {
  ...
  it("tool_progress appends notes to the card", () => {
    ...
    s = apply(s, { type: "tool_progress", sessionId: "s1", toolUseId: "t1", note: "step 1" });
    s = apply(s, { type: "tool_progress", sessionId: "s1", toolUseId: "t1", note: "step 2" });
...
describe("applyEventToMode — turn_end", () => {
  it("clears streaming state and attaches usage + stopReason", () => {
    let s = withTurnStart();
    s = apply(s, { type: "text_delta", sessionId: "s1", messageId: "m1", delta: "content" });
    s = apply(s, {
      type: "turn_end",
      sessionId: "s1",
      messageId: "m1",
  ...
  it("preserves messages written before turn_end", () => {
    let s = withTurnStart();
    s = apply(s, { type: "text_delta", sessionId: "s1", messageId: "m1", delta: "kept" });
    s = apply(s, {
      type: "turn_end",
      sessionId: "s1",
      messageId: "m1",
...
describe("mode isolation", () => {
  it("events applied to mode A do not affect mode B's state", () => {
    ...
    modeA = apply(modeA, { type: "turn_start", sessionId: "sA", messageId: "mA1", model: "m" });
    modeA = apply(modeA, { type: "text_delta", sessionId: "sA", messageId: "mA1", delta: "A text" });
    modeA = apply(modeA, {
      type: "tool_start",
      sessionId: "sA",
      toolUseId: "tA",
